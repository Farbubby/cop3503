...
do
	...
	# case 6 takes a long time
	if [[ TestCase$i == 'TestCase06' ]]; then
		for j in {1..5};
		do 
			# times out a case if it goes longer than 5s
			timeout --foreground 5s java TestCase$i > myoutput.txt 2> /dev/null
			if [[ $? -eq 124 ]]; then 
				echo "timed out"
				break
			fi
	...
	# we run the other cases 100 times
	else
		for j in {1..100};
		do
			# times out a case if it goes longer than 5s
